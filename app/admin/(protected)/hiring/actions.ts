...
export async function updateRoleAction(input: {
  ...
  revalidatePath(`/admin/hiring/${input.roleId}`);
  revalidatePath("/admin/hiring");
  return { ok: true };
}
...
export async function updateRoleStatusAction(
  ...
  if (error) return { ok: false, error: error.message.slice(0, 300) };
  revalidatePath("/admin/hiring");
  revalidatePath(`/admin/hiring/${roleId}`);
  return { ok: true };
}
...
export async function moveApplicationToStageAction(
  ...
  revalidatePath(`/admin/hiring/${stage.role_id as string}`);
  return { ok: true };
}
...
export async function createRoleAndRedirect(formData: FormData) {
  ...
  if (!companyId) {
    redirect(
      `/admin/hiring/new?error=${encodeURIComponent("Pick a company")}`,
    );
  }
  ...
  if (!result.ok) {
    redirect(`/admin/hiring/new?error=${encodeURIComponent(result.error)}`);
  }
  redirect(`/admin/hiring/${result.data.roleId}`);
}
